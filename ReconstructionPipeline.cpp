...
bool ReconstructionPipeline::reconstruct() {
  ...
  // 2. Lọc theo bán kính - radius=0.015, minNeighbors=2 (ít tích cực hơn)
  radiusOutlierFilter(0.015f, 2);

  // 3. VoxelGrid: leafSize=0.002 (lưới mịn hơn → giữ nhiều điểm hơn)
  //    Với object ~30cm, leafSize=0.002 cho ~150x150x150=3.3M voxel tối đa
  //    nhưng thực tế chỉ có điểm ở bề mặt → kết quả ~vài chục nghìn điểm
  voxelGridDownsample(0.002f);
...
// void ReconstructionPipeline::statisticalOutlierFilter(float meanK, float
// stdDevMulThresh) {
...
// void ReconstructionPipeline::radiusOutlierFilter(float radius, int
// minNeighbors) {
//     if (points3D.empty()) return;
//     PointCloudT::Ptr cloud = convertToPCLCloud(points3D, colors);
//     PointCloudT::Ptr filtered(new PointCloudT);  // ← Output riêng
