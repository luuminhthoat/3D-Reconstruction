...
bool ReconstructionPipeline::loadCameraParams(const QString &paramsFilePath) {
    ...
    if ((int)camParams.size() != numImages) {
        qWarning() << "Số camera đọc được" << camParams.size() << "≠ khai báo"
                   << numImages;
    }
    ...
    if (!camParams.empty()) {
        ...
        qDebug() << "  K: fx=" << c0.K.at<double>(0, 0)
                 << "fy=" << c0.K.at<double>(1, 1) << "cx=" << c0.K.at<double>(0, 2)
...
void ReconstructionPipeline::extractFeatures(int idx) {
    ...
    detector->detectAndCompute(gray, cv::noArray(), keypoints[idx],
                               descriptors[idx]);
}
...
void ReconstructionPipeline::matchFeatures(
    int idx1, int idx2, std::vector<cv::DMatch> &goodMatches) {
...
bool ReconstructionPipeline::estimatePoseFromMatches(
    ...
    if ((int)pts1.size() < 8)
        return false;
    cv::Mat mask;
    cv::Mat E = cv::findEssentialMat(pts1, pts2, K_fallback, cv::RANSAC, 0.999,
                                     1.0, mask);
    if (E.empty())
        return false;
...
void ReconstructionPipeline::doTriangulate(const cv::Mat &P0, const cv::Mat &P1,
    ...
    for (int i = 0; i < pts4D.cols; ++i) {
        ...
        if (std::abs(w) < 1e-9)
            continue;
        outPts.push_back(cv::Point3f((float)(pts4D.at<double>(0, i) / w),
...
// ------------------------------------------------------------------
void ReconstructionPipeline::filterOutliersByDensity(float radius,
                                                     int minNeighbors) {
    if (points3D.empty())
        return;
...
bool ReconstructionPipeline::reconstructWithEstimatedPose() {
    ...
    if (bestMatches < 100) {
        qWarning() << "Not found pair images base not good!";
        return false;
    }
    qDebug() << "Pair base:" << best_i << "-" << best_j
             << "matches=" << bestMatches;
    ...
    if (!estimatePoseFromMatches(pts_i, pts_j, R, t)) {
        qWarning() << "Can't estimate pose for base pair!";
        return false;
    }
    ...
    for (size_t k = 0; k < basePoints3D.size() && k < baseMatches.size(); ++k) {
        if (basePoints3D[k].z > 0.01f && basePoints3D[k].z < 10.0f) {
            baseCorr.push_back(
                {basePoints3D[k], baseMatches[k].queryIdx, baseMatches[k].trainIdx});
        }
    }
    ...
    for (size_t idx = 0; idx < images.size(); ++idx) {
        ...
        if (bestRef == -1 || maxMatches < 50) {
            qDebug() << "Ignore image" << idx << "don't have enough matches";
            continue;
        }
        ...
        if (!estimatePoseFromMatches(pts_cur, pts_ref, R_rel, t_rel)) {
            qDebug() << "Can't estimate pose for image " << idx;
            continue;
        }
        ...
        int added = 0;
        for (size_t k = 0; k < newPts.size() && k < matches.size(); ++k) {
            if (newPts[k].z <= 0 || newPts[k].z > 10.0f)
                continue;
        ...
        qDebug() << "Image" << idx << "add" << added << "points";
    }

    filterOutliersByDensity(0.02f, 3);
    qDebug() << "=== Total 3D points (estimated):" << points3D.size() << "===";
    return !points3D.empty();
}

PointCloudT::Ptr ReconstructionPipeline::convertToPCLCloud(
...
bool ReconstructionPipeline::reconstruct() {
    if (images.size() < 2) {
        qWarning() << "Need at least 2 images!";
        return false;
    }
    ...
    qDebug() << "Starting post-processing...";

    // filterOutliersByDensity(0.02f, 3);

    // 1. Statistical Outlier Filter - stddev=2.0 (less aggressive, keeps more
    // points)
    statisticalOutlierFilter(50, 2.0f);

    if (points3D.empty()) {
        qWarning() << "No points left after SOR, skipping further filters.";
        return true;
    }

    // 2. Lọc theo bán kính - radius=0.02, minNeighbors=2
    radiusOutlierFilter(0.02f, 2);

    if (points3D.empty()) {
        qWarning() << "No points left after ROR, skipping VoxelGrid.";
        return true;
    }
...
//     }
//   }
//   qDebug() << "SOR: keeps" << newPts.size() << "/" << n << "points";
//   points3D.swap(newPts);
//   colors.swap(newCols);
// }

// void ReconstructionPipeline::radiusOutlierFilter(float radius,
//                                                  int minNeighbors) {
//   if (points3D.empty())
//     return;
//   size_t n = points3D.size();
...
//     }
//   }
//   qDebug() << "ROR: keeps" << newPts.size() << "/" << n << "points";
//   points3D.swap(newPts);
//   colors.swap(newCols);
// }
...
//     newPts.push_back(cv::Point3f((float)(vd.sx * inv), (float)(vd.sy * inv),
//                                  (float)(vd.sz * inv)));
//     newCols.push_back(cv::Vec3b((uchar)(vd.sb * inv),   // B
//                                 (uchar)(vd.sg * inv),   // G
//                                 (uchar)(vd.sr * inv))); // R
//   }
//   qDebug() << "VoxelGrid: keep" << newPts.size() << "/" << points3D.size()
//            << "points";
//   points3D.swap(newPts);
//   colors.swap(newCols);
// }
...
// Helper: áp index list vào points3D và colors
static void applyIndices(const std::vector<int> &indices,
                         std::vector<cv::Point3f> &points3D,
                         std::vector<cv::Vec3b> &colors) {
    std::vector<cv::Point3f> newPts;
    std::vector<cv::Vec3b> newCols;
    newPts.reserve(indices.size());
    newCols.reserve(indices.size());
    for (int idx : indices) {
        newPts.push_back(points3D[idx]);
        newCols.push_back(colors[idx]);
    }
    points3D.swap(newPts);
    colors.swap(newCols);
}
...
// Dùng pcl::PointXYZ + filter(indices) tránh alignment issue của PointXYZRGB
void ReconstructionPipeline::statisticalOutlierFilter(float meanK,
                                                      float stdDevMulThresh) {
    if (points3D.empty())
        return;
    int k = static_cast<int>(meanK);
    if (k < 1)
        k = 1;
    if ((int)points3D.size() <= k) {
        qDebug() << "SOR: không đủ điểm (" << points3D.size() << "), bỏ qua.";
        return;
    }
    size_t n = points3D.size();

    auto cloud = buildXYZCloud(points3D);

    pcl::StatisticalOutlierRemoval<pcl::PointXYZ> sor;
    sor.setInputCloud(cloud);
    sor.setMeanK(k);
    sor.setStddevMulThresh(static_cast<double>(stdDevMulThresh));

    // Lấy indices thay vì copy cloud → không cần PointXYZRGB, tránh heap issue
    std::vector<int> kept;
    sor.filter(kept);

    applyIndices(kept, points3D, colors);
    qDebug() << "SOR: giữ lại" << points3D.size() << "/" << n << "điểm";
}
...
void ReconstructionPipeline::radiusOutlierFilter(float radius,
                                                 int minNeighbors) {
    if (points3D.empty() || radius <= 0.f || minNeighbors < 1)
        return;
    size_t n = points3D.size();
    ...
    ror.setRadiusSearch(static_cast<double>(radius));
    ...
    applyIndices(kept, points3D, colors);
    qDebug() << "ROR: giữ lại" << points3D.size() << "/" << n << "điểm";
}

// Giảm mẫu bằng Voxel Grid
// VoxelGrid không dùng KdTree nên dùng PointXYZRGB trực tiếp vẫn OK,
// nhưng để an toàn ta dùng PointXYZ + giữ màu trung bình riêng
void ReconstructionPipeline::voxelGridDownsample(float leafSize) {
    if (points3D.empty() || leafSize <= 0.f)
        return;
    size_t n = points3D.size();

    // Build XYZRGB cloud chỉ cho VoxelGrid (nó không dùng KdTree → safe)
    PointCloudT::Ptr cloud(new PointCloudT);
    cloud->resize(n);
    for (size_t i = 0; i < n; ++i) {
    ...
    if (!filtered->empty()) {
        points3D.clear();
        colors.clear();
        points3D.reserve(filtered->size());
        colors.reserve(filtered->size());
        for (const auto &p : filtered->points) {
            points3D.emplace_back(p.x, p.y, p.z);
            colors.emplace_back(p.b, p.g, p.r);
        }
    }
    qDebug() << "VoxelGrid: giữ lại" << points3D.size() << "/" << n << "điểm";
}
