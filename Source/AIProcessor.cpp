...
AIProcessor::AIProcessor() : isDetModelLoaded(false), isSegModelLoaded(false) {
  ...
  try {
    ...
    qWarning() << "Failed to initialize GPU provider, falling back to CPU:"
               << e.what();
  }
}
...
cv::Mat AIProcessor::runObjectDetection(const cv::Mat &inputImage) {
  ...
  for (int i = 0; i < 3; ++i) {
    inputTensorValues.insert(inputTensorValues.end(), (float *)channels[i].data,
                             (float *)channels[i].data +
